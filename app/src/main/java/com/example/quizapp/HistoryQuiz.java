...
public class HistoryQuiz extends AppCompatActivity implements View.OnClickListener{
    ...
    int score = 0;
    int totalQuestion = QuestionAnswer.history_questions.length;
    ...
    public void onClick(View view) {
        ...
        if (clickedButton.getId() == R.id.submit_btn) {
            if (selectedAnswer.equals(QuestionAnswer.history_correctAnswers[currentQuestionIndex])) {
                score++;
            }
    ...
    private void saveScore() {
        ...
        int previousScore = sharedPreferences.getInt("Quiz4Score", 0);

        if (score > previousScore) {
            SharedPreferences.Editor editor = sharedPreferences.edit();
            editor.putInt("Quiz4Score", score);
            editor.apply();
        }
    }
